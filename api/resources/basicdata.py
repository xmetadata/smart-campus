...
from models.basicdata import BasicData, ListSchema, BasicEdit
import datetime
import json
    ...
    def get(self, node_uuid):
        ...
        if node_uuid == "root":
            ...
            if root is None:
                ...
                node_uuid = root.node_uuid
        nodes = tm.find_node(node_uuid=node_uuid, many=True)
        return {"title" : root.title, "children" : ListSchema(many=True).dump(nodes).data}, 200
...
class BasicEdit():
    ...
    def post(self, node_uuid):
        ...
        load_data, errors = BasicEdit().loads(req_data)
        if errors:
            return errors, 400
